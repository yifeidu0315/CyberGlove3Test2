...
void Source(double* deltaT, double *jointAngles[5][3])
{
...
// 	int updateTimes = 0;
	...
	//{
        ...
        cout << "deltaT: " << glove->getLastUpdateTime() - baseT <<
			"\n";
        *deltaT = glove->getLastUpdateTime() - baseT;   // store timestamp
		...
		for (int finger = 0; finger < GHM::nbrFingers; finger++)
		{
			...
			{
                // store joint angles in 2d array
                jointAngles[finger][joint] = glove->getData((GHM::Fingers)finger, (GHM::Joints)joint); 
