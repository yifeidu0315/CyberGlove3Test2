/********************************************************************
Initialize a connection and update joint angle data and palm related data in radian from CyberGlove
...
double* Source()
{
	...
	// default: nbrFingers = 5, nbrJoints = 3
	const int m = GHM::nbrFingers + 1; // palm = 5
	const int n = GHM::nbrJoints + 1; // for palm, palmJoint=-1, palmArch = metacarpal, wristFlexion = proximal, wristAbduction = distal
	// finger tip = 3 = abduction, need to figure out the assignment as there are only 4 abduction sensors but five fingers

	rows = GHM::nbrFingers + 1; 
	cols = GHM::nbrJoints + 1; 
	static double GloveData[m][n];
	double* ptrGloveData = GloveData[0];

	// Get finger joint angles and the finger abductions
	cout << "Glove: \n";
	for (int finger = 0; finger < m - 1; finger++)
	{
		cout << finger << " ";
	...
	// Get palm joint angles
	for (int palmjoint = 0; palmjoint < n; palmjoint++)
	{
		GloveData[m - 1][palmjoint] = glove->getData((GHM::HandParts)(m - 1), (GHM::Joints)palmjoint);
	}
	cout << "Data stored" << "\n";
	// wait for 100ms
